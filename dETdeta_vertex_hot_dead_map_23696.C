...
*/
std::set<std::tuple<int, int>> ihcal_hot_dead_map_23696;
std::set<std::tuple<int, int>> ohcal_hot_dead_map_23696;

const float centrality_binning[11] = {0.0,2.0,27.0,66.0,131.0,228.0,368.0,573.0,860.0,1247.0,250000.0};

void dETdeta_vertex_hot_dead_map_23696() {	

	TDatabasePDG *_pdg = new TDatabasePDG();

	TFile *out = new TFile("run23696_time_vertex_hotdeadmap.root","RECREATE");
    ...
    TString wildcardPath = TString::Format("%sevents_20231211_p004_23696_data_cor_*.root", inputDirectory);
    ...
    while (reader.Next()) {
		...
		for (int i = 0; i < *m_simtwrmult_cemc; i++) {
		    ...
		    auto it = hot_dead_map_23696.find(hot_tower);
		    if (it != hot_dead_map_23696.end()) { continue; }
			if (m_simtwr_cemc_ishot[i]) { 
				tuple<int, int> new_hot_tower = make_tuple(m_simtwr_cemc_ieta[i], m_simtwr_cemc_iphi[i]); 
				hot_dead_map_23696.insert(new_hot_tower);
				continue; 
			}
		...
		for (int i = 0; i < *m_simtwrmult_ihcal; i++) {
		    ...
		    auto it = ihcal_hot_dead_map_23696.find(hot_tower);
		    if (it != ihcal_hot_dead_map_23696.end()) { continue; }
			if (m_simtwr_ihcal_ishot[i]) { 
				tuple<int, int> new_hot_tower = make_tuple(m_simtwr_ihcal_ieta[i], m_simtwr_ihcal_iphi[i]); 
				ihcal_hot_dead_map_23696.insert(new_hot_tower);
				continue; 
			}
		...
		for (int i = 0; i < *m_simtwrmult_ohcal; i++) {
		    ...
		    auto it = ohcal_hot_dead_map_23696.find(hot_tower);
		    if (it != ohcal_hot_dead_map_23696.end()) { continue; }
			if (m_simtwr_ohcal_ishot[i]) { 
				tuple<int, int> new_hot_tower = make_tuple(m_simtwr_ohcal_ieta[i], m_simtwr_ohcal_iphi[i]); 
				ohcal_hot_dead_map_23696.insert(new_hot_tower);
				continue; 
			}
	...
	std::cout << "emcal_hot_dead_map_23696 = {";
	for (const auto& tuple : hot_dead_map_23696) {
        std::cout << "{" << std::get<0>(tuple) << ", " << std::get<1>(tuple) << "}, ";
    }
    std::cout << std::endl;

    std::cout << "ihcal_hot_dead_map_23696 = {";
	for (const auto& tuple : ihcal_hot_dead_map_23696) {
        std::cout << "{" << std::get<0>(tuple) << ", " << std::get<1>(tuple) << "}, ";
    }
    std::cout << std::endl;

    std::cout << "ohcal_hot_dead_map_23696 = {";
	for (const auto& tuple : ohcal_hot_dead_map_23696) {
        std::cout << "{" << std::get<0>(tuple) << ", " << std::get<1>(tuple) << "}, ";
    }
    std::cout << std::endl;
