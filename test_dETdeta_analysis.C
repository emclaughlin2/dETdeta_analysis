...
void test_dETdeta_analysis(int dataormc = 0, int reweighting = 1, int central = 0, const char* generator = "") {	

	TDatabasePDG *_pdg = new TDatabasePDG();

	string filename = "dETdeta_analysis_23726_calibtest1";
	...
	TH2F* h_2D_ihcal_calib = new TH2F("h_2D_ihcal_calib","",24,0.,24.,64,0.,64.);
  	...
  	TH2F* h_test_2D_emcal_calibT = new TH2F("h_test_2D_emcal_calibT","",96,0.,96.,256,0.,256.);
	...
	TH1F* h_test_eT_emcal = new TH1F("h_test_eT_emcal","",220,-1.1,1.1);
    ...
    if (dataormc && !strcmp(generator, "epos")) {
    	...
    	//TString wildcardPath = "/sphenix/user/egm2153/calib_study/detdeta/runsimana0/output/evt/events_testcalib1_data_cor_0.root";
    	//chain.Add(wildcardPath);
    	const char* inputDirectory = "/sphenix/user/egm2153/calib_study/detdeta/runsimana0/output/evt/";
	    for (int i = 0; i < 100; i++) {
	    	TString wildcardPath = TString::Format("%sevents_testcalib1_p007_23726_data_cor_%d.root", inputDirectory, i);
	    	chain.Add(wildcardPath);
	    }
    } else {
	...
	float hijing_cent[] = {0.0,941.0,3020.0,4478.0,6418.0,8983.0,12221.0,16172.0,21010.0,26857.0,33720.0,41673.0,50856.0,61295.0,73376.0,87385.0,103226.0,121633.0,143473.0,250000.0};
    ...
    //for (Long64_t entry = 0; entry < 100000; ++entry) {
		...
		float E_ihcal[24] = {0};
  		...
  		if (m_vtx[2] <= -10.0 || m_vtx[2] >= 10.0) { continue; }
		...
		for (int i = 0; i < m_simtwrmult_cemc; i++) {
			...
			if (m_simtwr_cemc_ieta[i] < 8 || m_simtwr_cemc_ieta[i] > 88) continue;
			if ((m_simtwr_cemc_iphi[i] >= 216 && m_simtwr_cemc_iphi[i] <= 239) || (m_simtwr_cemc_iphi[i] >= 128 && m_simtwr_cemc_iphi[i] <= 143) 
				|| (m_simtwr_cemc_iphi[i] >= 160 && m_simtwr_cemc_iphi[i] <= 175) || (m_simtwr_cemc_iphi[i] >= 188 && m_simtwr_cemc_iphi[i] <= 207)) {
				h_test_eT_emcal->Fill(m_simtwr_cemc_eta[i], m_simtwr_cemc_e[i]*vz_weight/cosh(m_simtwr_cemc_eta[i]));
				h_test_2D_emcal_calibT->Fill(m_simtwr_cemc_ieta[i], m_simtwr_cemc_iphi[i], m_simtwr_cemc_e[i]*vz_weight/cosh(m_simtwr_cemc_eta[i]));
			}
		}
	...
	h_test_2D_emcal_calibT->Scale(1.0/totalweights);
	h_test_eT_emcal->Scale(1.0/totalweights);
	h_test_eT_emcal->Scale(1.0/0.01);
