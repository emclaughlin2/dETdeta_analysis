...
std::vector<float> tight_centrality_bin; // edit for tight centrality
...
void fill_hot_dead_map_eta_bin_centers(float minus_z, float plus_z) {
	...
	//TFile *hotdeadfile = new TFile(TString::Format("/sphenix/user/egm2153/calib_study/detdeta/analysis/Run2024/run54912_hotdeadmap_z_%d_%d_new_2024p007.root", int(floor(minus_z)), int(floor(plus_z))), "READ");
	...
	TFile *hotdeadfile = new TFile("/sphenix/user/egm2153/calib_study/detdeta/analysis/Run2024/run54912_hotdeadmap_z_-20_20_new_calib_12_12_24_withzscc_ana450_2024p009.root", "READ"); 
	...
	//TFile *etabinfile = new TFile(TString::Format("/sphenix/user/egm2153/calib_study/detdeta/analysis/Run2024/run54912_hotdeadmap_z_%d_%d_new_2024p007.root", int(floor(minus_z)), int(floor(plus_z))), "READ"); 
	//TFile *etabinfile = new TFile("/sphenix/user/egm2153/calib_study/detdeta/analysis/Run2024/run54912_hotdeadmap_z_-20_20_new_2024p007.root", "READ"); 
	TFile *etabinfile = new TFile("/sphenix/user/egm2153/calib_study/detdeta/analysis/Run2024/run54912_hotdeadmap_z_-20_20_new_calib_12_12_24_withzscc_ana450_2024p009.root", "READ"); 
...
void fill_zvertex_centrality(int dataormc, const char* generator) {
	...
	float mc_cent[20];
	float data_cent[20];
	float mc_tight_cent[100]; // edit for tight centrality
	if (dataormc) {
		...
		vertextree->SetBranchAddress("mc_centrality_tight", mc_tight_cent); // edit for tight centrality
...
void dETdeta_test_multi_run_analysis(const char* generator = "", float minus_z = -2, float plus_z = 2, int dataormc = 0, int reweighting = 1, int central = 0, int min_cent = 0, int max_cent = 5, int zs = 0, int zs_value = 10, int time = 0, const char* emsyst = "", const char* ihsyst = "", const char* ohsyst = "", const char* opt_tag = "", const char* subdirectory = "") {
	...
	TH1F* h_emcal_eta = new TH1F("h_emcal_eta","",emcal_num_bins, emcal_bin_edges);
	TH1F* h_eta_emcal = new TH1F("h_eta_emcal","",emcal_num_bins, emcal_bin_edges);
	...
	TH1F* h_ihcal_eta = new TH1F("h_ihcal_eta","",ihcal_num_bins, ihcal_bin_edges);
	TH1F* h_eta_ihcal = new TH1F("h_eta_ihcal","",ihcal_num_bins, ihcal_bin_edges);
	...
	TH1F* h_ohcal_eta = new TH1F("h_ohcal_eta","",ohcal_num_bins, ohcal_bin_edges);
	TH1F* h_eta_ohcal = new TH1F("h_eta_ohcal","",ohcal_num_bins, ohcal_bin_edges);
	...
	TH1F* h_calo_eta = new TH1F("h_calo_eta","",calo_num_bins, calo_bin_edges);
	TH1F* h_eta_calo = new TH1F("h_eta_calo","",calo_num_bins, calo_bin_edges);
    ...
    for (Long64_t entry = 0; entry < 10000; ++entry) {
		...
		*/
		...
		//std::cout << entry << " " << m_centbin << std::endl;
		...
		for (int i = 0; i < m_simtwrmult_cemc; i++) {
			...
			E_emcalbin[m_simtwr_cemc_ieta[i]] += m_simtwr_cemc_e[i]*vz_weight/cosh(m_simtwr_cemc_eta[i]);
			h_emcal_eta->Fill(m_simtwr_cemc_eta[i], vz_weight);
			h_calo_eta->Fill(m_simtwr_cemc_eta[i], vz_weight);
		...
		for (int i = 0; i < m_simtwrmult_ihcal; i++) {
			...
			h_ihcal_eta->Fill(m_simtwr_ihcal_eta[i], vz_weight);
			h_calo_eta->Fill(m_simtwr_ihcal_eta[i], vz_weight);
		...
		for (int i = 0; i < m_simtwrmult_ohcal; i++) {
			...
			h_ohcal_eta->Fill(m_simtwr_ohcal_eta[i], vz_weight);
			h_calo_eta->Fill(m_simtwr_ohcal_eta[i], vz_weight);
		...
		for (int i = 0; i < 24; i++) {
			h_eT_eta_ihcal->Fill(eta_ihcal[i],E_ihcal[i]);
			h_eT_eta_ohcal->Fill(eta_ohcal[i],E_ohcal[i]);
			h_eT_eta_calo->Fill(calo_eta_bin_centers[i],E_emcal[i]+E_ihcal[i]+E_ohcal[i]);
			//h_eta_emcal->Fill(eta_emcal[i],vz_weight);
			h_eta_ihcal->Fill(eta_ihcal[i],vz_weight);
			h_eta_ohcal->Fill(eta_ohcal[i],vz_weight);
			//h_eta_calo->Fill(calo_eta_bin_centers[i],vz_weight);
			h_eT_eta_ihcal_profile->Fill(eta_ihcal[i],E_ihcal[i]);
			h_eT_eta_ohcal_profile->Fill(eta_ohcal[i],E_ohcal[i]);
			if (E_emcal[i] != 0) {
				h_eT_eta_emcal->Fill(emcal_eta_bin_centers[i],E_emcal[i]);
				h_eT_eta_emcal_profile->Fill(emcal_eta_bin_centers[i],E_emcal[i]);
			}
			if (E_calo[i] != 0) {
				h_eT_eta_calo->Fill(calo_eta_bin_centers[i],E_calo[i]);
				h_eT_eta_calo_profile->Fill(calo_eta_bin_centers[i],E_calo[i]);
			}
	...
	h_emcal_eta->Scale(1.0/totalweights);
	h_eta_emcal->Scale(1.0/totalweights);
	for (int i = 1; i <= emcal_num_bins; ++i) {
        ...
        h_emcal_eta->SetBinContent(i, h_emcal_eta->GetBinContent(i) / bin_width);
        h_emcal_eta->SetBinError(i, h_emcal_eta->GetBinError(i) / bin_width);
        //h_eta_emcal->SetBinContent(i, h_eta_emcal->GetBinContent(i) / bin_width);
        //h_eta_emcal->SetBinError(i, h_eta_emcal->GetBinError(i) / bin_width);
        if (dataormc) {
	...
	h_ihcal_eta->Scale(1.0/totalweights);
	h_eta_ihcal->Scale(1.0/totalweights);
	for (int i = 1; i <= ihcal_num_bins; ++i) {
        ...
        h_ihcal_eta->SetBinContent(i, h_ihcal_eta->GetBinContent(i) / bin_width);
        h_ihcal_eta->SetBinError(i, h_ihcal_eta->GetBinError(i) / bin_width);
        //h_eta_ihcal->SetBinContent(i, h_eta_ihcal->GetBinContent(i) / bin_width);
        //h_eta_ihcal->SetBinError(i, h_eta_ihcal->GetBinError(i) / bin_width);
        if (dataormc) {
	...
	h_ohcal_eta->Scale(1.0/totalweights);
	h_eta_ohcal->Scale(1.0/totalweights);
	for (int i = 1; i <= ohcal_num_bins; ++i) {
        ...
        h_ohcal_eta->SetBinContent(i, h_ohcal_eta->GetBinContent(i) / bin_width);
        h_ohcal_eta->SetBinError(i, h_ohcal_eta->GetBinError(i) / bin_width);
        //h_eta_ohcal->SetBinContent(i, h_eta_ohcal->GetBinContent(i) / bin_width);
        //h_eta_ohcal->SetBinError(i, h_eta_ohcal->GetBinContent(i) / bin_width);
        if (dataormc) {
    ...
    h_calo_eta->Scale(1.0/totalweights);
    h_eta_calo->Scale(1.0/totalweights);
	for (int i = 1; i <= calo_num_bins; ++i) {
        ...
        h_calo_eta->SetBinContent(i, h_calo_eta->GetBinContent(i) / bin_width);
        h_calo_eta->SetBinError(i, h_calo_eta->GetBinError(i) / bin_width);
        //h_eta_calo->SetBinContent(i, h_eta_calo->GetBinContent(i) / bin_width);
        //h_eta_calo->SetBinError(i, h_eta_calo->GetBinError(i) / bin_width);
        if (dataormc) {
