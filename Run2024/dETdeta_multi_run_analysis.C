    ...
    //for (Long64_t entry = 0; entry < 10000; ++entry) {
		...
		*/
		...
		for (int i = 0; i < m_simtwrmult_cemc; i++) {
		    ...
		    if (m_simtwr_cemc_iphi[i] < 80 || m_simtwr_cemc_iphi[i] >= 140) { continue; } // EDIT FOR SMALL UNIFORM ACCEPTANCE
		...
		for (int i = 0; i < m_simtwrmult_ihcal; i++) {
		    ...
		    if (m_simtwr_ihcal_iphi[i] < 20 || m_simtwr_ihcal_iphi[i] >= 35) { continue; }
		...
		for (int i = 0; i < m_simtwrmult_ohcal; i++) {
		    ...
		    if (m_simtwr_ohcal_iphi[i] < 20 || m_simtwr_ohcal_iphi[i] >= 35) { continue; }
