...
void dETdeta_multi_run_analysis(const char* generator = "", float minus_z = -2, float plus_z = 2, int dataormc = 0, int reweighting = 1, int central = 0, int min_cent = 0, int max_cent = 5, int zs = 0, int zs_value = 10, int time = 0, const char* emsyst = "", const char* ihsyst = "", const char* ohsyst = "", const char* opt_tag = "", const char* subdirectory = "", int closure = 0) {
	...
	string filename = "/sphenix/user/egm2153/calib_study/detdeta/analysis/Run2024/ohcal_radius_scaling/";
	...
	if (!strcmp(generator,"hijing_ohcal_radius_reweight") || !strcmp(generator,"hijing")) {
		fill_zvertex_centrality(dataormc, "reweight_hijing_2024");
	} else if (!strcmp(generator,"epos_ohcal_radius_reweight") || !strcmp(generator,"epos") || !strcmp(generator,"epos_magnet") || !strcmp(generator,"epos_no_magnet")) {
    ...
    if (dataormc && !strcmp(generator, "epos")) {
	    ...
	    for (int i = 0; i < 1000; i++) {
	    	TString wildcardPath = TString::Format("%sevents_epos_run14_epos_cor_%d.root", inputDirectory, i);
	    ...
	    const char* inputDirectory = "/sphenix/tg/tg01/commissioning/CaloCalibWG/egm2153/detdeta_run24auau/";
	    for (int i = 0; i < 1000; i++) { 
	    	TString wildcardPath = TString::Format("%sevents_hijing_run14_mc_cor_%d.root", inputDirectory, i); 
	    	chain.Add(wildcardPath);
	    }
	...
	} else if (dataormc && !strcmp(generator, "hijing_ohcal_radius_reweight")) {
	    const char* inputDirectory = "/sphenix/tg/tg01/commissioning/CaloCalibWG/egm2153/detdeta_run24auau/";
	    int data_start = 0; int data_end = 1000;
	    for (int i = data_start; i < data_end; i++) { 
    		TString wildcardPath = TString::Format("%sevents_hijing_run14_ohcal_radius_reweight_mc_cor_%d.root", inputDirectory, i); 
	    	chain.Add(wildcardPath);
	    }
	} else if (dataormc && !strcmp(generator, "epos_ohcal_radius_reweight")) {
	    const char* inputDirectory = "/sphenix/tg/tg01/commissioning/CaloCalibWG/egm2153/detdeta_run24auau/";
	    int data_start = 0; int data_end = 1000;
	    for (int i = data_start; i < data_end; i++) { 
    		TString wildcardPath = TString::Format("%sevents_epos_run14_ohcal_radius_reweight_epos_cor_%d.root", inputDirectory, i); 
	    	chain.Add(wildcardPath);
	    }
	} else if (dataormc && !strcmp(generator, "epos_magnet")) {
	    const char* inputDirectory = "/sphenix/user/egm2153/calib_study/detdeta/eposrun/magnet_condor/";
	    int data_start = 0; int data_end = 1000;
	    for (int i = data_start; i < data_end; i++) { 
    		TString wildcardPath = TString::Format("%sOutDir%d/epos_sim_output.root", inputDirectory, i); 
	    	chain.Add(wildcardPath);
	    }
	} else if (dataormc && !strcmp(generator, "epos_no_magnet")) {
	    const char* inputDirectory = "/sphenix/user/egm2153/calib_study/detdeta/eposrun/no_magnet_condor/";
	    int data_start = 0; int data_end = 1000;
	    for (int i = data_start; i < data_end; i++) { 
    		TString wildcardPath = TString::Format("%sOutDir%d/epos_sim_output.root", inputDirectory, i); 
	    	chain.Add(wildcardPath);
